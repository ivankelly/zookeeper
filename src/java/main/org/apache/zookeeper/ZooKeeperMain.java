...
public class ZooKeeperMain {
    ...
    protected void connectToZK(String newHost) throws InterruptedException, IOException {
        ...
        String fdClassName = cl.getOption("fd." + fdName);
        String fdPrefix = "fd." + fdName + ".";
        
        FailureDetector fd = null;
        try {
            fd = new FailureDetectorFactory().createFd(
                    fdName, fdClassName, cl.parseOptionsStartingWith(fdPrefix));
        } catch (Exception e) {
            throw new IllegalArgumentException(
                    "No failure detection method instance could be created, " +
                    "please define one using the -fd option. " +
                    "Available failure detection methods are: " +
                    "[fixed, sliced, chen, bertier, phiaccrual]", e);
        }
