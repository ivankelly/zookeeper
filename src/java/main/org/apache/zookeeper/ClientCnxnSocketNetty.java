...
public class ClientCnxnSocketNetty extends ClientCnxnSocket {
    ...
    protected boolean doWrites(List<Packet> pendingQueue) {
        boolean written = false;
        synchronized (outgoingQueue) {
            while (!outgoingQueue.isEmpty() && channel.isWritable()) {
                Packet p = outgoingQueue.getFirst();
                ByteBuffer pbb = p.bb;
                ChannelFuture write = channel.write(ChannelBuffers
                        .copiedBuffer(pbb));
                if (p.header != null
                        && p.header.getType() == OpCode.closeSession) {
